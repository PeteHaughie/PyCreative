...
def main():
    ...
    @window.event
    def on_close():
        # close event from window manager should exit the app loop
        _exit_app()
        return True
    ...
    def on_draw():
        window.clear()
        # pyglet origin is bottom-left; we drew with top-left origin in Skia demo,
        # so flipping may be required depending on the path used. For our FBO path
        # we used BottomLeft origin; adjust if necessary.
        img.blit(0, 0)

    print('Entering manual event loop. Close the window or press Ctrl-C to exit.')

    # Manual, interruptible loop instead of pyglet.app.run() which in some
    # environments can ignore signals or be difficult to interrupt.
    running = True
    ...
    # ensure our signal handlers stop the loop
    try:
        signal.signal(signal.SIGINT, lambda *_: _stop_running())
        signal.signal(signal.SIGTERM, lambda *_: _stop_running())
    except Exception:
        pass

    start = time.time()
    try:
            while running and (time.time() - start) < 30.0:
                # tick timers and events
                pyglet.clock.tick()
                try:
                    window.dispatch_events()
                except Exception:
                    # dispatch_events can raise if window was closed concurrently
                    _stop_running()
                    break
                ...
                window.clear()
                try:
                    if use_direct_gl_draw and zero_ok:
                        # draw the shared GL texture directly (zero-copy)
                ...
                try:
                    ...
                    # if flip fails, stop and clean up
                    _stop_running()
                    break

                # small sleep to limit CPU and approximate 60fps
                time.sleep(1.0 / 60.0)
