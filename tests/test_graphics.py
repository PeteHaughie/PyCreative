...
def test_rect_calls(monkeypatch):
    dummy = DummySurface()
    monkeypatch.setattr('pygame.draw.rect', lambda surf, color, rect, width=0: surf.calls.append(('rect', color, rect, width)))
    s = Surface(dummy)
...
def test_ellipse_calls(monkeypatch):
    dummy = DummySurface()
    monkeypatch.setattr('pygame.draw.ellipse', lambda surf, color, rect, width=0: surf.calls.append(('ellipse', color, rect, width)))
    s = Surface(dummy)
    s.ellipse(10,20,30,40,color=(1,2,3),width=1)
    # x-w/2, y-h/2, w, h
    assert dummy.calls == [('ellipse', (1,2,3), (10-30/2,20-40/2,30,40), 1)]

def test_line_calls(monkeypatch):
    dummy = DummySurface()
    monkeypatch.setattr('pygame.draw.line', lambda surf, color, start, end, width=1: surf.calls.append(('line', color, start, end, width)))
    s = Surface(dummy)
