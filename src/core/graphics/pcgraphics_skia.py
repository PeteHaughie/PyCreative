...
class PCGraphicsSkia:
    def __init__(self, width: int, height: int, engine: Optional[Any] = None):
        ...
        # Try to create a GPU-backed Skia surface; fall back to CPU raster
        try:
            import skia
        ...
        if self._surf is not None:
            try:
                ...
                try:
                    ...
                    replay_to_skia_canvas(list(self._recording), canvas)
                except Exception:
    ...
    def rect(self, x, y, w, h):
        self._recording.append({'op': 'rect', 'x': float(x), 'y': float(y), 'w': float(w), 'h': float(h)})

    def ellipse(self, x, y, w, h):
        self._recording.append({'op': 'ellipse', 'x': float(x), 'y': float(y), 'w': float(w), 'h': float(h)})
