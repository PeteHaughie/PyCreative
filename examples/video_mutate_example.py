...
from pycreative.math import math
...
class VideoSketch(Sketch):
    ...
    def draw(self):
        self.clear(0)
        self.w = math.sin(self.t * 2 * math.pi / 60) * 100 + self.width / 2
        self.h = math.cos(self.t * 2 * math.pi / 60) * 100 + self.height / 2
        self.video(self.player, 0, 0, int(self.w), int(self.h))
