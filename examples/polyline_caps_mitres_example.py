...
class PolylineCapsMitresSketch(Sketch):
    def setup(self):
        self.size(600, 400)
        self.set_title("Polyline Stroke Caps & Mitres Example")
        self.bg = (255, 255, 255)
    ...
    def draw(self):
        ...
        for i, cap in enumerate(self._caps):
            ...
            self.line(x0, y, x1, y)
            self.text(f"cap: {cap}", x0, y - 30, color=(40,40,40))
            y += 60
        ...
        for i, join in enumerate(self._joins):
            ...
            self.polyline([(180, y), (300, y - 50), (420, y)])
            self.text(f"join: {join}", x0, y - 10, color=(40,40,40))
            y += 40
