"""
// P_2_1_1_04.pde
//
// Generative Gestaltung, ISBN: 978-3-87439-759-9
// First Edition, Hermann Schmidt, Mainz, 2009
// Hartmut Bohnacker, Benedikt Gross, Julia Laub, Claudius Lazzeroni
// Copyright 2009 Hartmut Bohnacker, Benedikt Gross, Julia Laub, Claudius Lazzeroni
//
// http://www.generative-gestaltung.de
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/**
 * shapes in a grid, that are always facing the mouse
 *
 * MOUSE
 * position x/y        : position to face
 *
 * KEYS
 * 1-7                 : choose shapes
 * arrow up/down       : scale of shapes
 * arrow left/right    : additional rotation of shapes
 * c                   : toggle. color mode
 * d                   : toggle. size depending on distance
 * g                   : toggle. grid resolution
 * s                   : save png
 */


PShape currentShape;

int tileCount = 10;
float tileWidth, tileHeight;
float shapeSize = 50;
float newShapeSize = shapeSize;
float shapeAngle = 0;
float maxDist;

color shapeColor = color(0, 130, 164);
int fillMode = 0;
int sizeMode = 0;


void setup(){
  size(600, 600);
  background(255);
  smooth();

  tileWidth = width/float(tileCount);
  tileHeight = height/float(tileCount);
  maxDist = sqrt(sq(width)+sq(height));

  currentShape = loadShape("module_1.svg");
}


void draw(){

  background(255);
  smooth();

  for (int gridY=0; gridY<tileCount; gridY++) {
    for (int gridX=0; gridX<tileCount; gridX++) {

      float posX = tileWidth*gridX + tileWidth/2;
      float posY = tileHeight*gridY + tileWidth/2;

      // calculate angle between mouse position and actual position of the shape
      float angle = atan2(mouseY-posY, mouseX-posX) + radians(shapeAngle);

      if (sizeMode == 0) newShapeSize = shapeSize;
      if (sizeMode == 1) newShapeSize = shapeSize*1.5-map(dist(mouseX,mouseY,posX,posY),0,500,5,shapeSize);
      if (sizeMode == 2) newShapeSize = map(dist(mouseX,mouseY,posX,posY),0,500,5,shapeSize);

      if (fillMode == 0) currentShape.enableStyle();
      if (fillMode == 1) {
        currentShape.disableStyle();
        fill(shapeColor);
      }
      if (fillMode == 2) {
        currentShape.disableStyle();
        float a = map(dist(mouseX,mouseY,posX,posY), 0,maxDist, 255,0);
        fill(shapeColor, a);
      }
      if (fillMode == 3) {
        currentShape.disableStyle();
        float a = map(dist(mouseX,mouseY,posX,posY), 0,maxDist, 0,255);
        fill(shapeColor, a);
      }

      pushMatrix();
      translate(posX, posY);
      rotate (angle);
      shapeMode (CENTER);

      noStroke();
      shape(currentShape, 0,0, newShapeSize,newShapeSize);
      popMatrix();
    }
  }

}


void keyPressed() {
  if (key == 's' || key == 'S') saveFrame("P_2_1_1_04_##.png");
  if (key == 'c' || key == 'C') fillMode = (fillMode+1) % 4;
  if (key == 'd' || key == 'D') sizeMode = (sizeMode+1) % 3;

  if (key == 'g' || key == 'G') {
    tileCount = tileCount+5;
    if (tileCount > 20) {
      tileCount = 10;
    }
    tileWidth = width/float(tileCount);
    tileHeight = height/float(tileCount);
  }

  if (key == '1') currentShape = loadShape("module_1.svg");
  if (key == '2') currentShape = loadShape("module_2.svg");
  if (key == '3') currentShape = loadShape("module_3.svg");
  if (key == '4') currentShape = loadShape("module_4.svg");
  if (key == '5') currentShape = loadShape("module_5.svg");
  if (key == '6') currentShape = loadShape("module_6.svg");
  if (key == '7') currentShape = loadShape("module_7.svg");

  if (keyCode == UP) shapeSize +=5;
  if (keyCode == DOWN) shapeSize = max(shapeSize-5, 5);
  if (keyCode == LEFT) shapeAngle -=5;
  if (keyCode == RIGHT) shapeAngle +=5;
}

"""

import math


class Sketch:
    def setup(self):
        self.size(600, 600)
        self.window_title("P_2_1_1_04")
        self.tile_count = 10
        self.tile_width = self.width / float(self.tile_count)
        self.tile_height = self.height / float(self.tile_count)
        self.shape_size = 50
        self.new_shape_size = self.shape_size
        self.shape_angle = 0
        self.max_dist = (self.width ** 2 + self.height ** 2) ** 0.5
        self.shape_color = (0, 130, 164)
        self.fill_mode = 0
        self.size_mode = 0
        self.fill(0)
        self.current_shape = self.load_shape("module_1.svg")

    def draw(self):
        self.background(255)

        for grid_y in range(self.tile_count):
            for grid_x in range(self.tile_count):

                pos_x = self.tile_width * grid_x + self.tile_width / 2
                pos_y = self.tile_height * grid_y + self.tile_width / 2

                # calculate angle between mouse position and actual position of the shape
                angle = math.atan2(
                    self.mouse_y - pos_y, self.mouse_x - pos_x) + self.radians(self.shape_angle)

                if self.size_mode == 0:
                    self.new_shape_size = self.shape_size
                if self.size_mode == 1:
                    self.new_shape_size = self.shape_size * 1.5 - \
                        self.map(self.dist(self.mouse_x, self.mouse_y,
                                 pos_x, pos_y), 0, 500, 5, self.shape_size)
                if self.size_mode == 2:
                    self.new_shape_size = self.map(self.dist(
                        self.mouse_x, self.mouse_y, pos_x, pos_y), 0, 500, 5, self.shape_size)
                if self.fill_mode == 0:
                    self.current_shape.enable_style()
                if self.fill_mode == 1:
                    self.current_shape.disable_style()
                    self.fill(self.shape_color)
                if self.fill_mode == 2:
                    self.current_shape.disable_style()
                    a = self.map(self.dist(self.mouse_x, self.mouse_y,
                                           pos_x, pos_y), 0, self.max_dist, 255, 0)
                    # deconstruct color tuple
                    self.fill(*self.shape_color[:3], a)
                if self.fill_mode == 3:
                    self.current_shape.disable_style()
                    a = self.map(self.dist(self.mouse_x, self.mouse_y,
                                           pos_x, pos_y), 0, self.max_dist, 0, 255)
                    # deconstruct color tuple
                    self.fill(*self.shape_color[:3], a)

                self.push_matrix()
                self.translate(pos_x, pos_y)
                self.rotate(angle)
                self.shape_mode("CENTER")
                self.no_stroke()
                self.shape(self.current_shape, 0, 0,
                           self.new_shape_size, self.new_shape_size)
                self.pop_matrix()

    def key_released(self):
        if self.key == 's' or self.key == 'S':
            self.save_frame("P_2_1_1_04_##.png")
        if self.key in ('c', 'C'):
            self.fill_mode = (self.fill_mode + 1) % 4
        if self.key in ('d', 'D'):
            self.size_mode = (self.size_mode + 1) % 3

        if self.key in ('g', 'G'):
            self.tile_count += 5
            if self.tile_count > 20:
                self.tile_count = 10
            self.tile_width = self.width / float(self.tile_count)
            self.tile_height = self.height / float(self.tile_count)

        if self.key in ('1', '2', '3', '4', '5', '6', '7'):
            self.current_shape = self.load_shape(f"module_{self.key}.svg")
            print(f"Loaded shape: module_{self.key}.svg")

        if self.key_code == 'UP':
            print("UP key pressed")
            self.shape_size += 5
        if self.key_code == 'DOWN':
            print("DOWN key pressed")
            self.shape_size = max(self.shape_size - 5, 5)
        if self.key_code == 'LEFT':
            print("LEFT key pressed")
            self.shape_angle -= 5
        if self.key_code == 'RIGHT':
            print("RIGHT key pressed")
            self.shape_angle += 5
