...
https://github.com/generative-design/Code-Package-Processing-3.x/blob/master/01_P/P_1_2_3_04/P_1_2_3_04.pde
...
class Sketch:
    def setup(self):
        self.size(800, 800)  # this original sketch was P3D but we don't have 3D here - yet
        self.window_title("P_1_2_3_04")
    ...
    def draw(self):
        ...
        for i in range(self.color_count):
            if i % 2 == 0:
                ...
                self.saturation_values[i] = int(self.random(0, 20))
        ...
        for i in range(row_count - 1, -1, -1):
            ...
            for part in parts:
                ...
                if self.random(1.0) < 0.45:
                    x = self.map(sum_parts_now, 0, sum_parts_total, 0, self.width) + self.random(-10, 10)
                    y = row_height * i + self.random(-10, 10)
                    w = self.map(part, 0, sum_parts_total, 0, self.width) * -1 + self.random(-10, 10)
                    h = row_height * 1.5
                    self.begin_shape()
                    self.fill(0, 0, 0, 180)
                    self.vertex(x, y)
                    ...
                    self.fill(self.hue_values[index], self.saturation_values[index], self.brightness_values[index], 100)
                    self.vertex(x + w, y + h)
                    self.vertex(x, y + h)
    ...
    def key_released(self):
        if self.key == 's' or self.key == 'S':
            self.save_frame("P_1_2_3_04_##.png")
